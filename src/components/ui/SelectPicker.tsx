...
const SelectPicker = ({
  ...
  const handleToggle = () => {
    ...
    setIsOpen(!isOpen);
    if (!isOpen && searchable) setTimeout(() => searchInputRef.current?.focus(), 0);
  };
  ...
  return (
    <div className={cn("relative w-full", className)} ref={dropdownRef} {...props}>
      <button
        ref={triggerRef}
        type="button"
        className={cn(
          ...
          sizes[size],
          disabled ? "cursor-not-allowed bg-gray-50 opacity-60" : "hover:border-gray-400",
